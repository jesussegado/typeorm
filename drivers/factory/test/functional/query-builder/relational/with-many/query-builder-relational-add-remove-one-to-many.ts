    ...
    it("should add entity relation of a given entity by entity objects", () =>
        Promise.all(
            connections.map(async (connection) => {
                ...
                loadedPost1 = await connection.manager.findOne(Post, 1, {
                    relations: ["category"],
                });
                expect(loadedPost1!.category).to.be.null;

                loadedPost2 = await connection.manager.findOne(Post, 2, {
                    relations: ["category"],
                });
                expect(loadedPost2!.category).to.be.null;

                loadedPost3 = await connection.manager.findOne(Post, 3, {
                    relations: ["category"],
                });
                expect(loadedPost3!.category).to.be.null;
            })
    ...
    it("should handle addAndRemove method as well", () =>
        Promise.all(
            connections.map(async (connection) => {
                ...
                let loadedPost1 = await connection.manager.findOne(Post, 2, {
                    relations: ["category"],
                });
                expect(loadedPost1!.category).to.be.eql({
                    id: 3,
                ...
                loadedPost1 = await connection.manager.findOne(Post, 2, {
                    relations: ["category"],
                });
                expect(loadedPost1!.category).to.be.eql({
                    id: 3,
