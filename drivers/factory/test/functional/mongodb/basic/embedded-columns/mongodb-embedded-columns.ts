    ...
    it("should insert / update / remove entity with embedded correctly", () =>
        Promise.all(
            connections.map(async (connection) => {
                ...
                const loadedUpdatedPost = await postRepository.findOne({
                    title: "Updated post",
                });
                ...
                const removedPost = await postRepository.findOne({
                    title: "Post",
                });
                const removedUpdatedPost = await postRepository.findOne({
                    title: "Updated post",
                });
    ...
    it("should transform entity with nested embedded columns correctly", () =>
        Promise.all(
            connections.map(async (connection) => {
                ...
                post.counters.extraInformation.lastEdit.title =
                    "Old Post Title";
