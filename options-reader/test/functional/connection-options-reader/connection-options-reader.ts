...
describe("ConnectionOptionsReader", () => {
    ...
    it("properly loads config with entities specified", async () => {
        ...
        const options: ConnectionOptions = await connectionOptionsReader.get(
            "test-conn"
        );
    ...
    it("properly loads sqlite in-memory/path config", async () => {
        ...
        const inmemoryOptions: ConnectionOptions = await connectionOptionsReader.get(
            "memory"
        );
        expect(inmemoryOptions.database).to.equal(":memory:");
        const fileOptions: ConnectionOptions = await connectionOptionsReader.get(
            "file"
        );
    ...
    it.skip("properly loads config from .env file", async () => {
        ...
        const [
            fileOptions,
        ]: ConnectionOptions[] = await connectionOptionsReader.all();
        expect(fileOptions.database).to.have.string("test-js");
