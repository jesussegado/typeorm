    ...
    it("should correctly create table with all dependencies and revert creation", () =>
        Promise.all(
            connections.map(async (connection) => {
                ...
                const categoryTableIdColumn = categoryTable!.findColumnByName(
                    "id"
                );
    ...
    it("should correctly create table with different `Unique` definitions", () =>
        Promise.all(
            connections.map(async (connection) => {
                ...
                const descriptionColumn = table!.findColumnByName(
                    "description"
                );
                ...
                if (connection.driver instanceof SqliteDriver) {
                    ...
                    try {
                        ...
                        expect(e.message).equal(
                            "SQLITE_ERROR: no such column: rowid"
                        );
                    }

                    await queryRunner.dropTable("book2");
                    const book2TableIsGone = await queryRunner.getTable(
                        "book2"
                    );
