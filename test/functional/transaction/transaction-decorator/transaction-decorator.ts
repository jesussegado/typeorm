...
describe("transaction > method wrapped into transaction decorator", () => {
        ...
        Promise.all(
            connections.map(async (connection) => {
                ...
                const loadedPost = await connection.manager.findOne(Post, {
                    where: { title: "successfully saved post" },
                });
                ...
                const loadedCategory = await connection.manager.findOne(
                    Category,
                    { where: { name: "successfully saved category" } }
                );
    ...
    it("should rollback transaction if any operation in the method failed", () =>
        Promise.all(
            connections.map(async (connection) => {
                const post = new Post();
                post.title = "successfully saved post";
    ...
    it("should save even if second operation failed in method not wrapped into @Transaction decorator", () =>
        Promise.all(
            connections.map(async (connection) => {
                ...
                let throwError: any;
                try {
                    await controller.nonSafeSave.apply(controller, [
                        connection.manager,
                        post,
                        category,
                    ]);
    ...
    it("should inject repository and custom repository into method decorated with @Transaction", () =>
        Promise.all(
            connections.map(async (connection) => {
                const post = new Post();
                post.title = "successfully saved post";
                ...
                // call controller method
                const savedCategory = await controller.saveWithRepository.apply(
                    controller,
                    [post, category]
                );
    ...
    it("should execute all operations in the method in a transaction with a specified isolation", () =>
        Promise.all(
            connections.map(async (connection) => {
                const post = new Post();
                post.title = "successfully saved post";
                ...
                // call controller method
                await controller.saveWithNonDefaultIsolation.apply(controller, [
                    post,
                    category,
                ]);
