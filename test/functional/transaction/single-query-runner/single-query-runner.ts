    ...
    it("should execute all operations in the method in a transaction (#804)", () =>
        Promise.all(
            connections.map(async (connection) => {
                ...
                const loadedPost5 = await entityManager.findOne(Post, {
                    title: "Hello World",
                });
                expect(loadedPost5).to.be.undefined;
                await entityManager.queryRunner!.rollbackTransaction();

                const loadedPost6 = await entityManager.findOne(Post, {
                    title: "Hello World",
                });
